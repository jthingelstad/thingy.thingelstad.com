import { escapeHtml, renderMarkdown, safeMarkdownUrl } from './thingy-markdown.js';

function sourceAccentClass(kind) {
  const normalized = String(kind || '')
    .toLowerCase()
    .replace(/[\s-]+/g, '_');
  if (normalized === 'weekly_thing' || normalized === 'newsletter' || normalized === 'issue' || normalized === 'chunk')
    return 'is-wt';
...
function renderExperience(experience) {
  ...
  const itemHtml = items
    .map((item, index) => {
      ...
      const meta = [item.label, item.publish_date ? String(item.publish_date).slice(0, 10) : '']
        .filter(Boolean)
        .join(' · ');
...
function renderCuriosityMap(map) {
  ...
  const nodes = rawNodes.some((node) => node.kind === 'center')
    ? rawNodes
    : [{ ...rawNodes[0], kind: 'center' }, ...rawNodes.slice(1)];
  ...
  const sourceHtml = sources
    .map((source) => {
      ...
      const meta = escapeHtml(
        [source.label, source.publish_date ? String(source.publish_date).slice(0, 10) : ''].filter(Boolean).join(' · ')
      );
