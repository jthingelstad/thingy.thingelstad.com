...
function formatElapsedTime(seconds: unknown) {
...
function createAssistantMessageModel(options: AssistantMessageOptions = {}): AssistantMessageModel {
  counter += 1;
  return {
    ...
    status: signal<AssistantMessageStatus>(options.status || 'pending'),
