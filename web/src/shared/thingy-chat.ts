...
import { mountAssistantMessage } from './components/AssistantMessage.tsx';
import { effect } from '@preact/signals';
import { normalizeScopeParam } from './thingy-scope.ts';
import { createSourcePicker } from './thingy-source-picker.ts';
import { attachRailState } from './thingy-rail-state.ts';
import { createDictationController } from './thingy-voice.ts';
...
import { focusAuthEmail, mountAuthPanel } from './components/AuthPanel.tsx';
import { mountAccountMenu } from './components/AccountMenu.tsx';
import { mountComposerCount } from './components/ComposerCount.tsx';
import { mountComposerSubmit } from './components/ComposerSubmit.tsx';
...
function bootChat() {
  ...
  const clearChatButton = document.getElementById('librarian-clear-chat') as HTMLButtonElement;
  const curiosityMapButton = document.getElementById('thingy-curiosity-map') as HTMLButtonElement;
  const modeControl = document.getElementById('thingy-mode-control');
  const modeIconEl = document.getElementById('thingy-mode-icon');
  const modeSelect = document.getElementById('thingy-mode-select') as HTMLButtonElement;
  ...
  const mobileConversationMenuButton = document.getElementById('mobile-conversation-menu-button') as HTMLButtonElement;
  ...
  function renderModeControl() {
    ...
    const selectedMode = state.availableModes.some((mode) => mode.id === state.activeMode)
      ? state.activeMode
      : 'thingy';
    ...
    if (modeMenu) {
      modeMenu.innerHTML = state.availableModes
        .map((mode) => {
  ...
  function unmountChildren(parent: HTMLElement) {
    Array.from(parent.children).forEach((child) => {
      if (child instanceof HTMLElement && typeof child._thingyUnmount === 'function') child._thingyUnmount();
    });
  }
  ...
  const { addPromptActions, addResponseActions, stopSpeaking } = createChatMessageActions({
    submitFeedback: ({ requestId, reaction, comment }) =>
      actions.postStreamJson(
        '/feedback',
        {
  ...
  function toggleMobileConversationMenu(force?: boolean) {
  ...
  async function showCuriosityMap(center = '', options: ThingyOptions = {}) {
    ...
    const attachToCurrent = Boolean(
      options.attachToCurrent &&
      state.activeConversationId &&
      !actions.isLocalConversationId(state.activeConversationId)
    );
    ...
    try {
      ...
      trackTinylyticsEvent(
        'librarian.curiosity_map_success',
  ...
  async function startAgentWelcome() {
    if (
      !actions.token() ||
      interactionBusy() ||
      welcomeInFlightSignal.value ||
      welcomeShownThisVisit ||
      hasInitialPrompt
    )
      return;
    if (!(await actions.ensureFreshToken())) {
  ...
  messages.addEventListener('click', (event) => {
    ...
    const retryButton = target ? target.closest<HTMLButtonElement>('button[data-retry-prompt]') : null;
    ...
    const button = target
      ? target.closest<HTMLButtonElement>('button[data-experience-prompt], button[data-map-prompt]')
      : null;
    ...
    questionInput.focus();
    trackTinylyticsEvent('librarian.experience_prompt', button.closest('.thingy-experience-spark') ? 'spark' : 'trail');
  });
  ...
  if (modeSelect) {
    ...
    modeSelect.addEventListener('keydown', (event) => {
      ...
      if (event.key === 'ArrowDown' || event.key === 'Enter' || event.key === ' ') {
        ...
        openModeMenu();
        modeMenu?.querySelector<HTMLElement>('[aria-selected="true"]')?.focus();
      }
    });
  }
  if (modeMenu) {
