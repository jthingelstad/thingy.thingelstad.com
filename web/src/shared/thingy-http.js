...
async function postJsonRequest(options = {}) {
  ...
  const response = await window.fetch(`${baseUrl}${options.path || ''}`, {
    method: 'POST',
    headers: {
  ...
  const requestId = options.requestIdSource === 'data'
    ? dataRequestId || headerRequestId
    : headerRequestId || dataRequestId;
