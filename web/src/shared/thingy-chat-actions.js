...
import { isAuthError, scrubUrlParams } from './thingy-url.js';
...
function createChatActions(options = {}) {
  ...
  async function refreshStoredAuth(opts = {}) {
  ...
  async function refreshAccountProfile(opts = {}) {
    if (!token() || tokenExpired()) return false;
    const now = Date.now();
  ...
  function clearAuthState(config = {}) {
  ...
  async function submitAuthCheck(opts = {}) {
  ...
  function dedupeEmptyConversationDrafts(list = [], opts = {}) {
  ...
  function upsertConversationSummary(conversation, opts = {}) {
  ...
  async function postStreamingWelcome(model, scope, opts = {}) {
    if (!streamBase) {
      throw new Error('Thingy has not been connected to the archive stream API yet.');
    }
