...
function persistCollapsed(key, value) {
  ...
  } catch (error) { /* private mode, no-op */ }
}
...
function attachRailState(options = {}) {
  ...
  return {
    setMobileOpen: (open) => { mobileRailOpen.value = Boolean(open); },
    closeMobile: () => { mobileRailOpen.value = false; },
    setCollapsed: (collapsed) => { railCollapsed.value = Boolean(collapsed); },
